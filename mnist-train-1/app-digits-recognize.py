...
if train:
    ...
    correct_prediction = tf.equal(tf.argmax(y, 1), tf.argmax(y_, 1))
    accuracy = tf.reduce_mean(tf.cast(correct_prediction, "float"))
    print "acuracia: ", sess.run(accuracy, feed_dict={x: mnist.test.images, y_: mnist.test.labels})



if not os.path.exists("input/" + image):
    print("O arquivo input/" + image + " nao existe!")
    exit(1)

# read original image
color_complete = cv2.imread("input/" + image)

print("ler", "input/" + image)
# read the bw image
gray_complete = cv2.imread("input/" + image, 0)

# better black and white version
_, gray_complete = cv2.threshold(255-gray_complete, 128, 255, cv2.THRESH_BINARY | cv2.THRESH_OTSU)

cv2.imwrite("output/complementar.png", gray_complete)
    ...
    for cropped_height in range(100, 300, 20):
        for shift_x in range(0, width-cropped_width, cropped_width/4):
            for shift_y in range(0, height-cropped_height, cropped_height/4):
                ...
                cv2.imwrite("output/"+image+"_"+str(shift_x)+"_"+str(shift_y)+".png", gray)
                ...
                print("Predicao para ",(shift_x, shift_y, cropped_width))
                print("Posicao")
                print(top_left)
                print(bottom_right)
                print(actual_w_h)
...
cv2.imwrite("output/"+image+"_reconhecido.png", color_complete)
